...
export function Hemocytometer() {
  ...
  const [methodConfig, setMethodConfig] = useState<MethodConfigByType['hemocytometer'] | null>(null);
  ...
  useEffect(() => {
    async function loadMethodConfig() {
      try {
        ...
        if (!token) return;
        const configData = await api.get<{ config: MethodConfigByType['hemocytometer'] }>(
          '/api/method-config/hemocytometer',
          token
        );
  ...
  useEffect(() => {
    ...
    const defaults = methodConfig.params;
    setSharedSettings({
      squaresCounted: defaults.defaultSquaresCounted,
      dilutionFactor: defaults.defaultDilution,
    });
  }, [id, methodConfig]);
  ...
  function isWithinTolerance(value1: number, value2: number): boolean {
    ...
    const lowCountThreshold = params?.lowCountThreshold ?? 10;
    const lowCountTolerance = params?.lowCountTolerance ?? 5;
    const tolerancePercent = params?.tolerancePercent ?? 20;
    // Use configured low-count tolerance
    if (value1 < lowCountThreshold || value2 < lowCountThreshold) {
      return Math.abs(value1 - value2) <= lowCountTolerance;
    }
  ...
  const outOfTolerance = bothDone && (!calculations.rbcWithinTolerance || !calculations.tncWithinTolerance);
  const canSubmit = bothDone && !outOfTolerance && (isQC || performerAttestationChecked);
  const { rbcSquares, rbcDilution, tncSquares, tncDilution } = getEffectiveSettings();
  ...
  return (
    <div className="hemocytometer">
      ...
      <div className="sides-container">
        {/* Side 1 */}
        <div className={`side ${side1.isDone ? 'done' : ''} ${outOfTolerance ? 'error' : ''}`}>
        ...
        {/* Side 2 */}
        <div className={`side ${side2.isDone ? 'done' : ''} ${outOfTolerance ? 'error' : ''}`}>
          <div className="side-header">
            <h2>Side 2</h2>
            <label className="done-checkbox">
              <input
