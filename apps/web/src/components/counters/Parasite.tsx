...
import type { ParasiteData, ParasiteCalculations, ParasiteRecord, MethodConfigByType } from '@lab-counters/shared';
import './Counter.css';
...
export function Parasite() {
  ...
  const [methodConfig, setMethodConfig] = useState<MethodConfigByType['parasite'] | null>(null);
  ...
  useEffect(() => {
    async function loadMethodConfig() {
      try {
        ...
        if (!token) return;
        const configData = await api.get<{ config: MethodConfigByType['parasite'] }>(
          '/api/method-config/parasite',
          token
        );
