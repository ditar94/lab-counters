 ...
 */
methodConfigRouter.put('/:counterType', authorize('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const counterType = CountRecordTypeSchema.parse(req.params.counterType);
    ...
    // Update config
    const updated = await updateOrgMethodConfig(req.user!.orgId, counterType, normalizedConfig);
 ...
 */
methodConfigRouter.delete('/:counterType', authorize('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const counterType = CountRecordTypeSchema.parse(req.params.counterType);
