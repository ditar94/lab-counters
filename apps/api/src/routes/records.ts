...
import type { CountRecordType, MethodConfigByType, ParamsSnapshot, MethodParamsByType } from '@lab-counters/shared';
import { calculateResults } from '../services/calculations';

export const recordsRouter = Router();

function resolveMethodConfig<T extends CountRecordType>(
  counterType: T,
  snapshot?: ParamsSnapshot | null
): MethodConfigByType[T] {
  const defaults = getDefaultMethodConfig(counterType);
  if (!snapshot || !snapshot.params) {
    return defaults;
  }
  const method = (snapshot.methodId ?? defaults.method) as MethodConfigByType[T]['method'];
  const params = { ...defaults.params, ...(snapshot.params as MethodParamsByType[T]) };
  return { method, params } as MethodConfigByType[T];
}

function validateRawTallies(
  counterType: CountRecordType,
  rawTallies: unknown,
  methodConfig: MethodConfigByType[CountRecordType]
): void {
  if (counterType === 'fetal') {
    const data = rawTallies as { fields?: number[] };
    const expected = (methodConfig.params as { rbcFieldsCount?: number }).rbcFieldsCount;
    if (!Array.isArray(data.fields)) {
      throw new AppError(400, 'INVALID_TALLIES', 'Fetal fields are required');
    }
    if (expected && data.fields.length !== expected) {
      throw new AppError(400, 'INVALID_TALLIES', `Expected ${expected} RBC fields but received ${data.fields.length}`);
    }
    const fetalExpected = (methodConfig.params as { fetalFieldsCount?: number }).fetalFieldsCount;
    const fetalFieldsCounted = (rawTallies as { fetalFieldsCounted?: number }).fetalFieldsCounted;
    if (fetalExpected !== undefined && fetalFieldsCounted !== undefined && fetalFieldsCounted > fetalExpected) {
      throw new AppError(400, 'INVALID_TALLIES', `Fetal fields counted (${fetalFieldsCounted}) exceeds expected ${fetalExpected}`);
    }
  }
}
...
recordsRouter.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const methodConfig = resolveMethodConfig(body.type, paramsSnapshot);
    validateRawTallies(body.type, body.rawTallies, methodConfig);
    const calculations = calculateResults(body.type, body.rawTallies, methodConfig);
...
recordsRouter.patch('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (body.rawTallies) {
      ...
      const methodConfig = resolveMethodConfig(existing.type as CountRecordType, existing.paramsSnapshot as ParamsSnapshot | null);
      validateRawTallies(existing.type as CountRecordType, body.rawTallies, methodConfig);
      updateData.calculations = calculateResults(existing.type as CountRecordType, body.rawTallies, methodConfig);
    }
    if (body.status) {
...
recordsRouter.post('/:id/submit', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (existing.type === 'fetal') {
      const methodConfig = resolveMethodConfig(existing.type as CountRecordType, existing.paramsSnapshot as ParamsSnapshot | null);
      const expectedFields = (methodConfig.params as { fetalFieldsCount?: number }).fetalFieldsCount ?? 0;
      const countedFields = (existing.rawTallies as { fetalFieldsCounted?: number }).fetalFieldsCounted ?? 0;
      if (expectedFields > 0 && countedFields < expectedFields) {
        throw new AppError(400, 'INVALID_TALLIES', `Fetal fields counted (${countedFields}) must reach ${expectedFields} before submission`);
      }
    }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (talliesChanged && body.rawTallies) {
        ...
        const methodConfig = resolveMethodConfig(existing.type as CountRecordType, existing.paramsSnapshot as ParamsSnapshot | null);
        validateRawTallies(existing.type as CountRecordType, body.rawTallies, methodConfig);
        updateData.calculations = calculateResults(existing.type as CountRecordType, body.rawTallies as Record<string, unknown>, methodConfig);
      }
